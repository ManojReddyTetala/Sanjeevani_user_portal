...
exports.generateMedicalRecordPDF = generateMedicalRecordPDF;
const pdfkit_1 = __importDefault(require("pdfkit"));
function generateMedicalRecordPDF(patient, records, referrals) {
    return new Promise((resolve, reject) => {
        try {
            ...
            doc.moveDown(0.5);
            doc.fillColor('#334155').fontSize(8).font('Helvetica').text('This PDF is suitable as the document destination for a QR-code prototype. When you create the QR code, encode a secure URL that points to this PDF or to a protected document viewer—not the patient\'s raw medical information inside the QR itself.');
            doc.moveDown(0.5);
            doc.fillColor('#dc2626').fontSize(8.5).font('Helvetica-Bold').text('Important: For a real healthcare deployment, use authentication, access control, encryption, audit logging, consent/authorization controls, and a secure patient portal or document server. Do not place personal health information directly into a publicly readable QR code.');
            doc.moveDown(1.5);
            doc.fillColor('#dc2626').fontSize(11).font('Helvetica-Bold').text('DEMO / SAMPLE DATA\nNOT A REAL MEDICAL RECORD', { align: 'center' });
            doc.moveDown(0.5);
            doc.fillColor('#7f1d1d').fontSize(9).font('Helvetica-Bold').text('This document is intentionally fictional and must not be used for diagnosis, treatment, identification, insurance, employment, legal, or other real-world medical purposes.', { align: 'center' });
            addFooter(6);
            doc.end();
        }
        catch (err) {
