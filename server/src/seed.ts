...
export function seedDatabase() {
  ...
  const insertRes = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  resources.forEach((r) => insertRes.run(
    r.hospital_id, r.icu_beds, r.general_beds, r.occupied_beds, r.general_ward_beds, r.oxygen_cylinders, r.ambulances, r.doctors_on_duty, r.nurses_on_duty, r.icu_facility_status, r.opd_queue_count, r.opd_queue_status, r.status, new Date().toISOString()
  ));
