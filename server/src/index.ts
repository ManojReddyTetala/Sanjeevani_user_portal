...
import os from 'os';
import { getDatabase, User, AuditLog, Patient, Hospital, Doctor, HospitalResource, DiagnosticService, MedicalRecord, Referral } from './db';
import { seedDatabase } from './seed';
import { resolveLocationQuery, fetchLocationAutocomplete } from './services/location';
import { verifyQRTokenAndFetchRecords } from './services/medicalRecords';
import { generateMedicalRecordPDF } from './services/pdf';
import { processAiMessage } from './services/ai';
import { fetchGooglePlacesNearby } from './services/googlePlaces';
...
app.get('/api/config/network-ip', (req: Request, res: Response) => {
...
function generateSecureToken(): { token: string; expires_at: string } {
...
function recordAuditLog(
  req: Request,
  actor: { id: number; role: string; name: string } | null,
  action: string,
...
function authenticateToken(req: AuthRequest, res: Response, next: NextFunction) {
  ...
  const db = getDatabase();
  const user = db.prepare('SELECT * FROM users WHERE token = ?').get(token) as User | undefined;
...
app.post('/api/auth/login', (req: Request, res: Response) => {
  try {
    ...
    const db = getDatabase();
    const cleanName = rawInput.replace(/@.*$/, '').replace(/[._]/g, ' ').replace(/\b\w/g, (c: string) => c.toUpperCase());
    ...
    `).get(cleanName, rawInput) as Patient | undefined;
    ...
    `).get(patient?.id || 0, userEmail, cleanName) as User | undefined;

    if (!patient && user && user.patient_id) {
      patient = db.prepare('SELECT * FROM patients WHERE id = ?').get(user.patient_id) as Patient | undefined;
    }
...
app.get('/api/auth/me', authenticateToken, (req: AuthRequest, res: Response) => {
  try {
    ...
    const db = getDatabase();
    const patient = db.prepare('SELECT * FROM patients WHERE id = ?').get(req.user.patient_id || 1) as Patient | undefined;
...
app.get('/api/me/records', authenticateToken, (req: AuthRequest, res: Response) => {
  try {
    const db = getDatabase();
    const patientId = req.user?.patient_id || 1;
    const patient = db.prepare('SELECT * FROM patients WHERE id = ?').get(patientId) as Patient | undefined;

    if (!patient) return res.status(404).json({ error: { code: 'PATIENT_NOT_FOUND', message: 'Patient not found' } });

    const records = db.prepare('SELECT * FROM medical_records WHERE patient_id = ? ORDER BY id DESC').all(patient.id) as any[];
...
app.get('/api/me/referrals', authenticateToken, (req: AuthRequest, res: Response) => {
  try {
    const db = getDatabase();
    const patientId = req.user?.patient_id || 1;
...
app.put('/api/me/profile', authenticateToken, (req: AuthRequest, res: Response) => {
  try {
    ...
    const updatedPatient = db.prepare('SELECT * FROM patients WHERE id = ?').get(patientId) as Patient;
...
app.get('/api/users', authenticateToken, requireRole(['PHC_MASTER']), (req: AuthRequest, res: Response) => {
  try {
    const db = getDatabase();
...
app.post('/api/users', authenticateToken, requireRole(['PHC_MASTER']), (req: AuthRequest, res: Response) => {
  try {
...
app.put('/api/users/:id', authenticateToken, requireRole(['PHC_MASTER']), (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!oldUser) return res.status(404).json({ error: { code: 'USER_NOT_FOUND', message: 'User not found.' } });
...
app.patch('/api/users/:id/status', authenticateToken, requireRole(['PHC_MASTER']), (req: AuthRequest, res: Response) => {
  try {
    const userId = parseInt(req.params.id);
...
app.post('/api/hospitals', authenticateToken, requireRole(['PHC_MASTER']), (req: AuthRequest, res: Response) => {
  try {
...
app.put('/api/hospitals/:id', authenticateToken, requireRole(['PHC_MASTER', 'HOSPITAL_ADMIN']), (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!oldHosp) return res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Hospital not found.' } });
...
app.patch('/api/hospitals/:id/status', authenticateToken, requireRole(['PHC_MASTER']), (req: AuthRequest, res: Response) => {
  try {
    const hospId = parseInt(req.params.id);
    const { is_active } = req.body;
    const db = getDatabase();
...
app.post('/api/doctors', authenticateToken, requireRole(['PHC_MASTER', 'HOSPITAL_ADMIN']), (req: AuthRequest, res: Response) => {
  try {
...
app.put('/api/doctors/:id', authenticateToken, requireRole(['PHC_MASTER', 'HOSPITAL_ADMIN']), (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!oldDoc) return res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Doctor not found.' } });
...
app.get('/api/health-tracks', (req: Request, res: Response) => {
  try {
    const patientId = req.query.patient_id ? parseInt(req.query.patient_id as string) : 1;
    const db = getDatabase();

    const tracks = db.prepare('SELECT * FROM health_tracks WHERE patient_id = ? ORDER BY id ASC').all(patientId);

    const fullTracks = tracks.map((track: any) => {
      ...
      const parsedTasks = tasks.map((t: any) => {
        ...
        try {
          if (t.dependencies_json) dependencies = JSON.parse(t.dependencies_json);
        } catch (e) {}
...
app.get('/api/health-tracks/:id', (req: Request, res: Response) => {
  try {
    const trackId = parseInt(req.params.id);
    const db = getDatabase();

    const track = db.prepare('SELECT * FROM health_tracks WHERE id = ?').get(trackId);
    if (!track) return res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Health track not found' } });
...
app.post('/api/health-track-tasks/:id/complete', (req: Request, res: Response) => {
  try {
    ...
    const completedCount = allTasks.filter((t: any) => t.status === 'COMPLETED' || t.id === taskId).length;
...
app.patch('/api/doctors/:id/status', authenticateToken, requireRole(['PHC_MASTER', 'HOSPITAL_ADMIN']), (req: AuthRequest, res: Response) => {
  try {
    const docId = parseInt(req.params.id);
    const { is_active } = req.body;
    const db = getDatabase();
...
app.post('/api/diagnostics', authenticateToken, requireRole(['PHC_MASTER', 'HOSPITAL_ADMIN']), (req: AuthRequest, res: Response) => {
  try {
...
app.put('/api/diagnostics/:id', authenticateToken, requireRole(['PHC_MASTER', 'HOSPITAL_ADMIN']), (req: AuthRequest, res: Response) => {
  try {
...
app.delete('/api/diagnostics/:id', authenticateToken, requireRole(['PHC_MASTER', 'HOSPITAL_ADMIN']), (req: AuthRequest, res: Response) => {
  try {
    const diagId = parseInt(req.params.id);
    const db = getDatabase();
...
app.get('/api/audit-logs', authenticateToken, requireRole(['PHC_MASTER']), (req: AuthRequest, res: Response) => {
  try {
    const db = getDatabase();
...
app.get('/api/search/healthcare', (req: Request, res: Response) => {
  try {
    const q = (req.query.q as string || '').toLowerCase().trim();
    const db = getDatabase();
    const results: any[] = [];
...
app.get('/api/search/doctors', (req: Request, res: Response) => {
  try {
    const q = (req.query.q as string || '').toLowerCase().trim();
    const db = getDatabase();
    const results: any[] = [];
...
app.get('/api/search/diagnostics', (req: Request, res: Response) => {
  try {
    const q = (req.query.q as string || '').toLowerCase().trim();
    const db = getDatabase();
    const results: any[] = [];
...
app.get('/api/search/records', (req: Request, res: Response) => {
  try {
    const q = (req.query.q as string || '').toLowerCase().trim();
    const db = getDatabase();
    const results: any[] = [];
    ...
    const records = db.prepare(`
      ...
      ORDER BY id DESC
    `).all(`%${q}%`, `%${q}%`, `%${q}%`, `%${q}%`, `%${q}%`, `%${q}%`) as MedicalRecord[];
...
app.get('/api/search/referrals', (req: Request, res: Response) => {
  try {
    const q = (req.query.q as string || '').toLowerCase().trim();
    const db = getDatabase();
    const results: any[] = [];
...
app.get('/api/search/statistics', (req: Request, res: Response) => {
  try {
    const q = (req.query.q as string || '').toLowerCase().trim();
    const results: any[] = [];
...
app.get('/api/location/resolve', async (req: Request, res: Response) => {
  try {
    ...
    if (!q) return res.status(400).json({ error: { code: 'INVALID_INPUT', message: 'Location query text is required.' } });
    const locationData = await resolveLocationQuery(q);
...
app.get('/api/location/autocomplete', async (req: Request, res: Response) => {
  try {
    ...
    const suggestions = await fetchLocationAutocomplete(q);
...
app.post('/api/ai/chat', (req: Request, res: Response) => {
  try {
    ...
    if (!message) return res.status(400).json({ error: { code: 'INVALID_INPUT', message: 'Message text is required.' } });
    const aiRes = processAiMessage(message, latitude, longitude);
    res.json(aiRes);
  } catch (error) {
...
app.get('/api/health-records/access/:qrToken', (req: Request, res: Response) => {
  try {
    const qrToken = req.params.qrToken;
    const result = verifyQRTokenAndFetchRecords(qrToken);
    if (!result.valid) {
...
app.get('/api/patients/:uid/pdf', async (req: Request, res: Response) => {
  try {
    const uid = req.params.uid;
    const db = getDatabase();
    const patient = db.prepare('SELECT * FROM patients WHERE uid = ? OR id = ?').get(uid, uid) as any;
    ...
    const pdfBuffer = await generateMedicalRecordPDF(patient, records, referrals);
...
app.get('/api/hospitals/nearby', (req: Request, res: Response) => {
  try {
    const lat = parseFloat(req.query.lat as string) || 28.5672;
    const lng = parseFloat(req.query.lng as string) || 77.2100;
    const maxRadius = parseFloat(req.query.radius as string) || 50;
...
app.get('/api/hospitals/:id', (req: Request, res: Response) => {
  try {
    ...
    const hosp = db.prepare(`
      ...
      WHERE h.id = ?
    `).get(id);
    if (!hosp) return res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Hospital not found' } });
    res.json(hosp);
  } catch (error) {
...
app.post('/api/hospitals/:id/resources', authenticateToken, requireRole(['HOSPITAL_ADMIN', 'PHC_MASTER']), (req: AuthRequest, res: Response) => {
  try {
    ...
    if (icu === 0 && gen < 10) status = 'UNAVAILABLE';
    else if (icu < 5 || gen < 30) status = 'LIMITED';
...
app.get('/api/phc/:id/overview', (req: Request, res: Response) => {
  try {
    const phcId = parseInt(req.params.id) || 7;
    const db = getDatabase();

    const facility = db.prepare('SELECT * FROM hospitals WHERE id = ?').get(phcId);
    if (!facility) return res.status(404).json({ error: { code: 'NOT_FOUND', message: 'PHC facility not found' } });
...
app.put('/api/phc/:id/resources', (req: Request, res: Response) => {
  try {
    const phcId = parseInt(req.params.id) || 7;
    const {
      general_beds,
    ...
    if (gen - occ <= 0 && icu === 0) overallStatus = 'UNAVAILABLE';
    else if (gen - occ <= 2 || (icu > 0 && icu <= 1)) overallStatus = 'LIMITED';
...
app.put('/api/phc/:id/queue', (req: Request, res: Response) => {
  try {
    const phcId = parseInt(req.params.id) || 7;
    ...
    if (!queueStatus) {
      if (count <= 10) queueStatus = 'SHORT';
      else if (count <= 30) queueStatus = 'MODERATE';
      else queueStatus = 'LONG';
    }
...
app.get('/api/phc/:id/staff', (req: Request, res: Response) => {
  try {
    const phcId = parseInt(req.params.id) || 7;
    const db = getDatabase();
...
app.patch('/api/phc/:id/staff/:staffId/duty', (req: Request, res: Response) => {
  try {
    const phcId = parseInt(req.params.id) || 7;
    ...
    const staffMembers = db.prepare('SELECT * FROM phc_staff WHERE hospital_id = ?').all(phcId) as any[];
...
app.get('/api/phc/:id/medicines', (req: Request, res: Response) => {
  try {
    const phcId = parseInt(req.params.id) || 7;
    const db = getDatabase();
    const medicines = db.prepare('SELECT * FROM phc_medicines WHERE hospital_id = ? ORDER BY id ASC').all(phcId);
...
app.patch('/api/phc/:id/medicines/:medId/status', (req: Request, res: Response) => {
  try {
    const phcId = parseInt(req.params.id) || 7;
...
app.patch('/api/phc/:id/diagnostics/:diagId/status', (req: Request, res: Response) => {
  try {
    const phcId = parseInt(req.params.id) || 7;
...
app.post('/api/phc/records', (req: Request, res: Response) => {
  try {
    const {
      patient_uid,
      patient_id,
    ...
    if (!targetPatientId && patient_uid) {
      const patient = db.prepare('SELECT id FROM patients WHERE uid = ? OR qr_token = ?').get(patient_uid, patient_uid) as any;
    ...
    `).run(
      targetPatientId,
      hospital_name || 'Primary Health Centre (PHC) Peddapuram',
      record_type || 'Consultation',
      title || 'Primary OPD Consultation Record',
      diagnosis || '',
      notes || '',
...
app.post('/api/phc/referrals', (req: Request, res: Response) => {
  try {
    const {
      patient_id,
      patient_uid,
    ...
    if (!targetPatientId && patient_uid) {
      const p = db.prepare('SELECT id FROM patients WHERE uid = ? OR qr_token = ?').get(patient_uid, patient_uid) as any;
      if (p) targetPatientId = p.id;
    }

    if (!targetPatientId) targetPatientId = 1;
    ...
    `).run(
      ...
      required_facility || 'Specialist Evaluation & Advanced Care',
      clinical_notes || 'Patient referred from Primary Health Centre due to unavailability of specialist.',
      now,
      now
    );
...
app.get('/api/hospital/:id/overview', (req: Request, res: Response) => {
  try {
...
app.put('/api/hospital/:id/resources', (req: Request, res: Response) => {
  try {
    const hospitalId = parseInt(req.params.id) || 1;
    const {
      icu_beds,
    ...
    `).run(
      ...
      general_beds,
      occupied_beds,
      general_ward_beds || general_beds,
      oxygen_cylinders,
      ambulances,
      doctors_on_duty,
      nurses_on_duty,
...
app.get('/api/hospital/:id/equipment', (req: Request, res: Response) => {
  try {
    const hospitalId = parseInt(req.params.id) || 1;
    const db = getDatabase();
    const equipment = db.prepare('SELECT * FROM hospital_equipment WHERE hospital_id = ? ORDER BY id ASC').all(hospitalId);
...
app.patch('/api/hospital/:id/equipment/:eqId/status', (req: Request, res: Response) => {
  try {
    const hospitalId = parseInt(req.params.id) || 1;
...
app.get('/api/hospital/:id/supplies', (req: Request, res: Response) => {
  try {
    const hospitalId = parseInt(req.params.id) || 1;
    const db = getDatabase();
    const supplies = db.prepare('SELECT * FROM medical_supplies WHERE hospital_id = ? ORDER BY id ASC').all(hospitalId);
...
app.patch('/api/hospital/:id/supplies/:supId/status', (req: Request, res: Response) => {
  try {
    const hospitalId = parseInt(req.params.id) || 1;
...
app.get('/api/hospital/:id/nurse-tasks', (req: Request, res: Response) => {
  try {
    const hospitalId = parseInt(req.params.id) || 1;
    const db = getDatabase();
...
app.post('/api/hospital/:id/nurse-tasks', (req: Request, res: Response) => {
  try {
    const hospitalId = parseInt(req.params.id) || 1;
...
app.patch('/api/hospital/:id/nurse-tasks/:taskId/status', (req: Request, res: Response) => {
  try {
    const hospitalId = parseInt(req.params.id) || 1;
...
app.post('/api/hospital/referrals/recommend', (req: Request, res: Response) => {
  try {
    ...
    const ranked = hospitals.map((h) => {
      ...
      if (hasSpecialist) score += 30; else score -= 25;
      if (hasIcu) score += 20; else if (requires_icu) score -= 40;
      if (availableGeneralBeds > 5) score += 15;
...
// --- PATIENTS & RECORDS ---
app.get('/api/patients/:uid', (req: Request, res: Response) => {

  try {
    const identifier = req.params.uid;
    const db = getDatabase();
    const patient = db.prepare('SELECT * FROM patients WHERE uid = ? OR qr_token = ? OR id = ?').get(identifier, identifier, identifier) as Patient | undefined;

    if (!patient) return res.status(404).json({ error: { code: 'PATIENT_NOT_FOUND', message: 'Patient identity record not found.' } });
...
app.get('/api/patients/:uid/pdf', async (req: Request, res: Response) => {
  try {
    ...
    const records = db.prepare('SELECT * FROM medical_records WHERE patient_id = ? ORDER BY id DESC').all(patient.id) as any[];
    const pdfBuffer = await generateMedicalRecordPDF(patient, records, []);
...
app.get('/api/health-records/access/:qrToken', async (req: Request, res: Response) => {
  try {
    const { qrToken } = req.params;
    const db = getDatabase();
    const patient = db.prepare('SELECT * FROM patients WHERE qr_token = ? OR uid = ? OR id = ?').get(qrToken, qrToken, qrToken) as Patient | undefined || {
      id: 1,
      name: 'Manoj',
      uid: 'UID-IND-9842-7104',
      age: 28,
...
app.get('/api/patients/:uid/records', (req: Request, res: Response) => {
  try {
    const identifier = req.params.uid;
    const db = getDatabase();
    const patient = db.prepare('SELECT * FROM patients WHERE uid = ? OR qr_token = ? OR id = ?').get(identifier, identifier, identifier) as Patient | undefined;
...
app.post('/api/records', authenticateToken, requireRole(['DOCTOR', 'PHC_MASTER']), (req: AuthRequest, res: Response) => {
  try {
...
app.post('/api/referrals', authenticateToken, requireRole(['DOCTOR', 'HOSPITAL_ADMIN', 'PHC_MASTER']), (req: AuthRequest, res: Response) => {
  try {
...
app.patch('/api/referrals/:id/status', authenticateToken, requireRole(['DOCTOR', 'HOSPITAL_ADMIN', 'PHC_MASTER']), (req: AuthRequest, res: Response) => {
  try {
...
app.post('/api/emergency/requests', (req: Request, res: Response) => {
  try {
    ...
    const db = getDatabase();
    const facility = db.prepare('SELECT * FROM hospitals WHERE id = ?').get(facility_id) as any;
    if (!facility) {
...
app.get('/api/emergency/requests', (req: Request, res: Response) => {
  try {
    const db = getDatabase();
    const patientId = req.query.patient_id ? parseInt(req.query.patient_id as string) : null;
    const facilityId = req.query.facility_id ? parseInt(req.query.facility_id as string) : null;
...
app.post('/api/emergency/requests/:id/accept', (req: Request, res: Response) => {
  try {
...
app.post('/api/emergency/requests/:id/dispatch-ambulance', (req: Request, res: Response) => {
  try {
    const requestId = parseInt(req.params.id);
    ...
    `).run(
      ...
      eta_minutes || 6,
      phc_notes || 'Ambulance AMB-07 dispatched with oxygen & AED onboard.',
      now,
      requestId
    );
...
app.patch('/api/emergency/requests/:id/telemetry', (req: Request, res: Response) => {
  try {
    const requestId = parseInt(req.params.id);
    const {
      patient_lat, patient_lng, patient_accuracy_m,
      ambulance_lat, ambulance_lng, ambulance_speed_kmh, ambulance_heading, ambulance_accuracy_m,
      eta_minutes, distance_km
    } = req.body;
    const db = getDatabase();

    const oldReq = db.prepare('SELECT * FROM emergency_requests WHERE id = ?').get(requestId) as any;
    if (!oldReq) {
    ...
    db.prepare(`
      ...
      WHERE id = ?
    `).run(
      nextPatientLat, nextPatientLng, nextPatientAcc, now,
      nextAmbLat, nextAmbLng, nextAmbSpeed, nextAmbHead,
      nextAmbAcc, now, nextEta, nextDist,
      now, requestId
    );
...
app.patch('/api/emergency/requests/:id/ambulance-state', (req: Request, res: Response) => {
  try {
    const requestId = parseInt(req.params.id);
    ...
    `).run(
      ...
      ambStatus,
      overallStatus,
      eta_minutes !== undefined ? eta_minutes : oldReq.eta_minutes,
      phc_notes || `Ambulance status transitioned to ${lifecycle_state}`,
      now,
      now,
      requestId
    );
...
app.post('/api/emergency/requests/:id/refer', (req: Request, res: Response) => {
  try {
    const requestId = parseInt(req.params.id);
    ...
    `).run(
      ...
      required_facility || destination_hospital_name || 'AIIMS Delhi Trauma & Cardiac Unit',
      clinical_notes || `Emergency referral from PHC for patient ${oldReq.patient_name}`,
      now,
      now
    );
...
app.post('/api/emergency/requests/:id/resolve', (req: Request, res: Response) => {
  try {
    const requestId = parseInt(req.params.id);
    ...
    `).run(
      oldReq.patient_id,
      oldReq.facility_name || 'Primary Health Centre',
      record_title || '🚨 Emergency Triage & Stabilization Report',
      diagnosis || 'Acute Emergency Stabilized (Normal Sinus Rhythm restored, Vitals stable)',
      resolution_notes || oldReq.description || 'Emergency patient stabilized with oxygen support and medication.',
      JSON.stringify(prescription_items || [{ medicine: 'Aspirin 300mg / Sorbitrate 5mg', dosage: 'Stat dose given', duration: '1 day' }]),
      now,
      'Dr. Sunita Rani (PHC Medical Officer)'
    );
...
app.patch('/api/emergency/requests/:id/status', (req: Request, res: Response) => {
  try {
    const requestId = parseInt(req.params.id);
...
app.get('/api/hospital-os/:id/overview', (req: Request, res: Response) => {
  try {
    const hospitalId = parseInt(req.params.id);
    const db = getDatabase();

    const hospital = db.prepare('SELECT * FROM hospitals WHERE id = ?').get(hospitalId) as any;
    const resources = db.prepare('SELECT * FROM hospital_resources WHERE hospital_id = ?').get(hospitalId) as any;
    const emergencies = db.prepare('SELECT * FROM emergency_requests WHERE facility_id = ? AND status != "RESOLVED" AND status != "CANCELLED"').all(hospitalId) as any[];
    const nurseTasks = db.prepare('SELECT * FROM nurse_tasks WHERE hospital_id = ?').all(hospitalId) as any[];
    const labOrders = db.prepare('SELECT * FROM diagnostic_orders WHERE hospital_id = ?').all(hospitalId) as any[];
    const equipment = db.prepare('SELECT * FROM hospital_equipment WHERE hospital_id = ?').all(hospitalId) as any[];
    const medicines = db.prepare('SELECT * FROM phc_medicines WHERE hospital_id = ?').all(hospitalId) as any[];
    const bedUnits = db.prepare('SELECT * FROM bed_units WHERE hospital_id = ?').all(hospitalId) as any[];
    const escalations = db.prepare('SELECT * FROM escalations WHERE hospital_id = ? AND status = "ACTIVE"').all(hospitalId) as any[];
    ...
    const warnings: Array<{ type: 'CRITICAL' | 'WARNING' | 'INFO'; message: string; action: string }> = [];
    ...
    const lowStockMeds = medicines.filter((m: any) => m.stock_level === 'Low Stock' || m.status === 'LIMITED');
    if (lowStockMeds.length > 0) {
      warnings.push({
        type: 'WARNING',
        message: `🟠 Inventory Warning: ${lowStockMeds.length} essential medicines running low (${lowStockMeds.map((m: any) => m.name).slice(0, 2).join(', ')}).`,
    ...
    res.json({
      ...
      ambulances_total: 5,
      nurse_tasks_pending_count: nurseTasks.filter((t: any) => t.status === 'PENDING').length,
      lab_orders_pending_count: labOrders.filter((l: any) => l.status !== 'COMPLETED').length,
...
app.get('/api/hospital-os/:id/nurse-tasks', (req: Request, res: Response) => {
  try {
    const hospitalId = parseInt(req.params.id);
    const db = getDatabase();
    const tasks = db.prepare('SELECT * FROM nurse_tasks WHERE hospital_id = ? ORDER BY id DESC').all(hospitalId);
...
app.post('/api/hospital-os/:id/nurse-tasks', (req: Request, res: Response) => {
  try {
    const hospitalId = parseInt(req.params.id);
    const { patient_id, patient_name, bed_number, title, priority, assigned_nurse, shift, due_time } = req.body;
    const db = getDatabase();
    ...
    `).run(
      ...
      title,
      priority || 'ROUTINE',
      assigned_nurse || 'Sister Lakshmi Devi',
      shift || 'Morning',
      due_time || '12:00 PM',
      new Date().toISOString()
    );
...
app.patch('/api/hospital-os/:id/nurse-tasks/:taskId/complete', (req: Request, res: Response) => {
  try {
    const taskId = parseInt(req.params.taskId);
    const db = getDatabase();
...
app.post('/api/hospital-os/:id/escalate', (req: Request, res: Response) => {
  try {
    const hospitalId = parseInt(req.params.id);
    ...
    `).run(
      ...
      patient_id || 1,
      patient_name || 'Rahul Kumar',
      room_number || 'Room 102 (ICU Bed 2)',
      nurse_name || 'Sister Lakshmi Devi',
      doctor_name || 'Dr. Anil Kumar',
      priority || 'CRITICAL',
      reason || 'Patient vitals deteriorating: SpO2 dropping to 88%, acute dyspnea.',
      new Date().toISOString()
    );
...
app.get('/api/hospital-os/:id/lab-orders', (req: Request, res: Response) => {
  try {
    const hospitalId = parseInt(req.params.id);
    const db = getDatabase();
...
app.post('/api/hospital-os/:id/lab-orders', (req: Request, res: Response) => {
  try {
    const hospitalId = parseInt(req.params.id);
...
app.patch('/api/hospital-os/:id/lab-orders/:orderId/status', (req: Request, res: Response) => {
  try {
    ...
    `).run(
      status,
      result_summary || 'Test processed and verified by Clinical Pathologist.',
      report_url || '/reports/lab_verified.pdf',
      status === 'COMPLETED' ? now : null,
      orderId
    );

    const updated = db.prepare('SELECT * FROM diagnostic_orders WHERE id = ?').get(orderId) as any;
    ...
    if (status === 'COMPLETED' && updated) {
      ...
      `).run(
        ...
        updated.doctor_id || 1,
        updated.hospital_id,
        `🧪 ${updated.test_name} Result`,
        updated.result_summary || 'Verified Diagnostic Test Result',
        `Diagnostic report completed. Summary: ${updated.result_summary}`,
        now
      );
...
app.get('/api/hospital-os/:id/equipment', (req: Request, res: Response) => {
  try {
    const hospitalId = parseInt(req.params.id);
    const db = getDatabase();
...
app.patch('/api/hospital-os/:id/equipment/:eqId', (req: Request, res: Response) => {
  try {
    const eqId = parseInt(req.params.eqId);
    const { status, notes } = req.body;
    const db = getDatabase();

    db.prepare('UPDATE hospital_equipment SET status = ?, notes = ?, last_inspected = ? WHERE id = ?').run(
      status,
      notes || '',
      new Date().toISOString(),
      eqId
...
app.get('/api/hospital-os/:id/beds/grid', (req: Request, res: Response) => {
  try {
    const hospitalId = parseInt(req.params.id);
    const db = getDatabase();
...
app.post('/api/hospital-os/:id/beds/reserve', (req: Request, res: Response) => {
  try {
    const hospitalId = parseInt(req.params.id);
    ...
    if (!targetBedId) {
      ...
      const availBed = db.prepare('SELECT id FROM bed_units WHERE hospital_id = ? AND status = "AVAILABLE" LIMIT 1').get(hospitalId) as any;
...
app.post('/api/hospital-os/capability-check', (req: Request, res: Response) => {
  try {
    ...
    const hospital = db.prepare('SELECT * FROM hospitals WHERE id = ?').get(hospId) as any;
    const resources = db.prepare('SELECT * FROM hospital_resources WHERE hospital_id = ?').get(hospId) as any;
    const doctors = db.prepare('SELECT * FROM doctors WHERE hospital_id = ? AND is_active = 1').all(hospId) as any[];
    const equipment = db.prepare('SELECT * FROM hospital_equipment WHERE hospital_id = ?').all(hospId) as any[];

    const hasSpecialist = doctors.some((d: any) => d.specialty?.toLowerCase().includes((required_specialty || 'cardio').toLowerCase()));
    ...
    const hasCathLab = equipment.some((e: any) => e.name.toLowerCase().includes('cath') && e.status === 'OPERATIONAL');

    let score = 0;
    if (hasSpecialist) score += 30;
    if (hasIcu) score += 25;
    if (hasBed) score += 20;
    if (hasCathLab || !requires_cath_lab) score += 15;
    if (hasAmbulance) score += 10;

    const canHandle = score >= 80;

    // Alternative matching hospitals
    const allHospitals = db.prepare('SELECT * FROM hospitals WHERE id != ? AND is_active = 1').all(hospId) as any[];
    const matchHospitals = allHospitals.map((h: any) => ({
      id: h.id,
      name: h.name,
...
app.post('/api/patients/:uid/consent', (req: Request, res: Response) => {
  try {
    const uid = req.params.uid;
    const { doctor_name, hospital_name, scopes, duration_minutes } = req.body;
    const db = getDatabase();

    const patient = db.prepare('SELECT id FROM patients WHERE uid = ?').get(uid) as any;
    if (!patient) return res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Patient not found' } });
    ...
    `).run(
      ...
      JSON.stringify(scopes || ['REPORTS', 'PRESCRIPTIONS', 'FULL_HISTORY']),
      duration,
      now,
      expiresAt
    );
...
app.get('/api/patients/:uid/access-logs', (req: Request, res: Response) => {
  try {
    const uid = req.params.uid;
    const db = getDatabase();
...
app.post('/api/hospital-os/staff-ai', (req: Request, res: Response) => {
  try {
    ...
    if (q.includes('cardiologist') || q.includes('doctor')) {
      const docs = db.prepare('SELECT name, specialty, is_on_duty FROM doctors WHERE hospital_id = ?').all(hospId) as any[];
      ...
      const resData = db.prepare('SELECT * FROM hospital_resources WHERE hospital_id = ?').get(hospId) as any;
